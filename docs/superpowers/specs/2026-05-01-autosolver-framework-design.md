...
- The initial validity model follows the baseline: each task and courier is used at most once.
- Although the problem statement mentions assigning one order to multiple couriers, the provided baseline does not do this. The first framework keeps the baseline constraint and leaves multi-courier assignment as a future extension.
