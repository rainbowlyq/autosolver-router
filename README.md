...
在外卖配送系统中，需要把配送订单分配给骑手。每个订单-骑手组合都有一个预计算的分数和接单概率；同一个任务可以同时指派给多个骑手，但骑手有概率拒绝订单，因此任务是否被覆盖要按至少一个骑手接单的概率计算，而不是只要分配就认为已覆盖。系统需要在多个目标之间取得平衡：**最大化期望接单订单数量**、**最小化期望系统总分数**，并通过**智能合单**（将多个订单合并给同一骑手）来优化资源利用率。
...
| 列名 | 含义 |
|------|------|
| `task_id_list` | 任务编号列表。单个任务如 `T0001`，合单任务如 `T0001,T0002`（表示一个骑手同时送两个单） |
| `courier_id` | 骑手编号 |
...
1. **同一个任务可以指派给多个骑手** — 多个骑手都可能拒单，任务覆盖率按接单概率计算
2. **每个骑手最多被分配一次** — 同一个骑手编号只能在方案中出现一次
3. **只能使用候选集中存在的分配** — 不能凭空编造不存在的任务-骑手组合
...
**行为**：将所有候选按 `total_score` 升序排列，然后依次遍历，只要该候选的骑手未被占用，就将其加入方案。同一个任务可以被多个骑手重复指派，覆盖贡献由接单概率合成。
