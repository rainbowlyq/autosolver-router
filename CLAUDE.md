...
- **禁止 `typing` 导入**：评测环境不支持导入 `typing` 库，所有类型注解需避免依赖 `typing`（如 `List`、`Tuple`、`Optional` 等），可使用 Python 3.6 内置语法（如 `list`、`tuple`）或不使用类型注解。目前项目中通过在打包脚本中使用`strip-hints`来移除类型注解，以确保兼容评测环境，开发环境暂时允许使用 `typing` 以提升代码可读性。
- **禁止 `namedtuple` 导入**：评测环境不支持导入和使用`namedtuple`，目前采用原生类和 `__slots__` 来定义数据结构。
