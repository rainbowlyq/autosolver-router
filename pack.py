...
class PackResult:
    __slots__ = ("output_path", "strip_hints_applied", "strip_hints_error")

    def __init__(self, output_path, strip_hints_applied, strip_hints_error):
        self.output_path = output_path
        self.strip_hints_applied = strip_hints_applied
        self.strip_hints_error = strip_hints_error
...
def pack_solver(output_path=None, template_path=None, strip_hints=True, project_root=None):
    ...
    output = Path(output_path) if output_path is not None else default_output_path(root)
...
def default_output_path(project_root=None, now=None):
    root = Path(project_root or PROJECT_ROOT)
    current_time = now or datetime.now()
    return root / "dist" / ("solver_%s.py" % current_time.strftime("%m%d%H%M%S"))
...
def build_arg_parser():
    ...
    parser.add_argument(
        ...
        "-o",
        default=None,
        help="Packed solver output path. Default: dist/solver_<mmddhhMMss>.py",
    )
